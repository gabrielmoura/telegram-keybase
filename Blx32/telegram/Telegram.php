 ...
 * Agradeço a Gabriele Grillo <gabry.grillo@alice.it> pôs parte desse script eu copiei dela.
 */
/**
 * Class Telegram
...
class Telegram extends TelegramErrorLogger
{
    ...
    protected $folder = "";
    protected $botan_token = "";

    /**
     * Telegram constructor.
     * \param $data[bot_id|loder_log|adms|bot_name|botan]
     */
    public function __construct($data)
    {
        $this->bot_id = $data['bot_id'];
        if ($data['folder_log'] != null): $this->folder = $data['folder_log'];endif;
        if ($data['adms'] != null):$this->adm = $data['adms'];endif;
        if ($data['bot_name'] != null):$this->bot_name = $data['bot_name'];endif;
        if ($data['botan'] != null):$this->botan_token = $data['botan'];endif;
...
/// Get the number of updates
    ...
    /**
     * Escreve os dados passados em um determinado arquivo.
     * \param $file_and_folder Diretório/File.extension
     * \param $content Conteudo
     * \return bool Se escrito
     */
    ...
    /**
     * Atenção com este comando se faz necessário usar o !==
     * \param $command Comando a ser executado.
     * \param $input Só se necessário. self::Text()
     * \return bool|string Retorna parametros se houver.
     */
    public function command($command, $input = null)
    {
        if ($input == null):$text = self::Text();
        else:$text = $input;endif;
        if (mb_substr($text, 0, strlen($command) + 1) == '/' . $command):
            if (mb_substr($text, 0, strlen($command . $this->bot_name) + 2) == '/' . $command . '@' . $this->bot_name):
                $sting = trim(mb_substr($text, strlen($command . '@' . $this->bot_name) + 1));
            else:
                $sting = trim(mb_substr($text, strlen($command) + 1));
            endif;
        else:
            return false;
        endif;
        return mb_convert_encoding($sting, 'UTF-8');
    }

    /**
     * APP Botan
     * \param string $event_name = 'Message'
     */
    public function Botan($event_name = 'Message')
    {
        $Botan = new Botan($this->botan_token);
        $Botan->track(self::getData()['message'], $event_name);
    }

    public function AppMetrica($event_name = 'Message')
    {
        $app = new AppMetricaApi();
        $label = $app->createLabel($event_name);
        $app->moveApp2Label($this->botan_token, $label);
    }

    /**
     * Retorna um array com os valores separados por space.
     * @param $enter
     * @return array
     */
    public function param($enter)
    {
        return explode(" ", $enter);
        /* foreach ($var as $item):
             return $item;
         endforeach;*/
    }
